...
class StockApiXueqiu(object):
    ...
    def get_his_k_data(
            ...
            end_date_str: str,
            period: str = 'day',
            indicators: List[str] = ['kline'],
            return_column: List[str] = ['symbol', 'date', 'volume', 'open', 'high', 'low', 'close', 'chg', 'percent']
    ) -> pandas.DataFrame:
        """
        :param symbol: 代码
        ...
        :param indicators: default [kline] all [kline,pe,pb,ps,pcf,market_capital,agt,ggt,balance]
    ...
    def get_k_data(
        ...
        start_of_today = datetime_type.strptime(date_str, "%Y-%m-%d") + timedelta(minutes=1)
        end_of_today = start_of_today + timedelta(days=1) - timedelta(minutes=1)
